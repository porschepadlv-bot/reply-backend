...
function normalizeCategory(category) { 
 ...
 const allowed = ["dating", "relationship", "friendship", "family", "work", "teen"];  if (allowed.includes(value)) return value; 
...
function buildSystemPrompt(category, intent, goal) {  const intentRuleText = intentRules(category, intent)  .map((rule) => `- ${rule}`) 
 .join("\\n"); 
 let categoryRules = "";
 if (category === "dating") { 
 categoryRules = ` 
...
- If the message suggests coffee, drinks, dinner, hanging out, compliments on appearance, attraction, or anything outside profession- Do not sound interested, enthusiastic, playful, warm, flirtatious, personal, or emotionally open. 
...
- If intent is closure in family category, tone must still remain serious, calm, and emotionally grounded. - Prioritize emotional intelligence, calm tone, understanding, accountability when needed, maturity, empathy, and clear communicatio- If the message is negative, critical, tense, dismissive, or hurtful: acknowledge it, do not deflect, do not joke, do not minimize,- Responses should feel mature, real, emotionally aware, and appropriate for a real relationship conversation. - Stay grounded in the actual situation. 
...
- If the incoming message is personal or socially suggestive, every reply must politely maintain a professional boundary. - Never accept coffee, drinks, dinner, hanging out, or personal invitations in work category. 
` 
 : ""; 
 return ` 
...
- For relationship category, assume the message is about a real relationship issue unless the message clearly proves otherwise. - In relationship category, do not interpret criticism as flirting, teasing, or banter. 
- In family category, default to a serious, respectful, emotionally aware tone. 
- If the family message suggests rejection, exclusion, or hurt, do not respond with jokes, surprise humor, or casual banter. - If category is family and intent is closure: do not use casual language like "that's a bummer", "wow", or "haha"; do not ask playf- If the incoming message is critical, tense, dismissive, or insulting, do not respond playfully. 
${workExtra} 
Return only a valid JSON array of 5 strings. 
`.trim();
} 
...
app.post("/reply", async (req, res) => { 
 ...
 const rawCategory = req.body?.category || req.body?.context || req.body?.message || "";  const category = normalizeCategory(rawCategory); 
